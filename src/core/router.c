...
int catzilla_router_init(catzilla_router_t* router) {
    ...
    router->root = malloc(sizeof(catzilla_route_node_t));
        ...
        if (router->routes[i]) {
            ...
            if (router->routes[i]->param_names) {
                for (int j = 0; j < router->routes[i]->param_count; j++) {
                    free(router->routes[i]->param_names[j]);
                }
...
static catzilla_route_node_t* catzilla_router_create_node(catzilla_router_t* router) {
    catzilla_route_node_t* node = malloc(sizeof(catzilla_route_node_t));
    if (!node) return NULL;
    ...
    node->children = malloc(sizeof(catzilla_route_node_t*) * node->child_capacity);
    node->child_segments = malloc(sizeof(char*) * node->child_capacity);

    // Initialize handlers arrays
    node->handler_capacity = 4;
    node->handlers = malloc(sizeof(catzilla_route_t*) * node->handler_capacity);
    node->methods = malloc(sizeof(char*) * node->handler_capacity);

    if (!node->children || !node->child_segments || !node->handlers || !node->methods) {
        free(node->children);
        free(node->child_segments);
        free(node->handlers);
        free(node->methods);
        free(node);
        return NULL;
...
uint32_t catzilla_router_add_route(catzilla_router_t* router,
    ...
    if (route->param_count > 0) {
        route->param_names = malloc(sizeof(char*) * route->param_count);
        ...
        for (int i = 0; i < segment_count; i++) {
            if (catzilla_router_is_param_segment(segments[i])) {
                ...
                route->param_names[param_idx] = malloc(strlen(param_name) + 1);
...
static int catzilla_router_add_to_trie(catzilla_router_t* router, catzilla_route_t* route,
        ...
        if (is_param) {
            ...
            if (!child) {
                ...
                if (current->child_count >= current->child_capacity) {
                    ...
                    catzilla_route_node_t** new_children = realloc(current->children,
                                                                  sizeof(catzilla_route_node_t*) * new_capacity);
                    char** new_segments = realloc(current->child_segments,
                ...
                current->child_segments[current->child_count] = malloc(strlen(segments[i]) + 1);
    ...
    if (current->handler_count >= current->handler_capacity) {
        ...
        catzilla_route_t** new_handlers = realloc(current->handlers,
                                                 sizeof(catzilla_route_t*) * new_capacity);
        char** new_methods = realloc(current->methods,
                                    sizeof(char*) * new_capacity);
    ...
    current->methods[current->handler_count] = malloc(strlen(route->method) + 1);
